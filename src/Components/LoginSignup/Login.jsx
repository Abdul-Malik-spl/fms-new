...
export const Login = () => {
   let inpRefs=useRef([])
let [userData,setUserData]=useState({adminName:"",adminPass:"",errors:{adminName:false,adminPass:false}})
let [rememberMe,setRememberMe]=useState(false)
let classes={lblcls:"",inpcls:"adminTxtBox",errCls:"errormsg",rowcon:"adminFormRowCon"}
let formData=[{rowCon:classes.rowcon,
                lblCls:classes.lblcls,
                inpId:"adminName",
                lblName:"Admin Name/Mail*",
    ...
    <div className='d-flex align-items-center justify-content-center h-75 ' style={{overflow:"hidden"}}>
        ...
        <div className="rememberforgetsec">
         <span className='d-flex gap-2'>
          <input type='checkbox' id='cbremember'onChange={()=>setRememberMe(prev=>!prev)}/>
          <label htmlFor='cbremember' className='lblRemeberMe'>Remember Me</label></span>
          <span className='fpasslink'>Forget Password?</span>
        </div>
        <div className={classes.rowcon} >
          <button onClick={AdminLogin} className='btn btn-primary'>Login</button>
        </div>
        </form>
    </div>
  )
